...
<body class="easyui-layout">
	...
	<div data-options="region:'center',title:'',border:false" style="padding: 5px;">
		...
		<table id="dg" title="出库明细" class="easyui-datagrid" data-options="fit:true,rownumbers:true,toolbar:'#toolbar',singleSelect:true,pagination:true,border:true">
			<thead>
				<tr>
					...
					<th data-options="sortable:true,field:'begin_time',width:'180px'">开始时间</th>
					<th data-options="sortable:true,field:'end_time',width:'180px'">结束时间</th>
					<th data-options="sortable:true,field:'G_TYPE',width:'100px'">零件用途</th>
					<th data-options="sortable:true,field:'DELIVERY_REC',width:'150px'">目的地代码</th>
			</thead>
		</table>
	</div>
	...
	<div id="toolbar">
		<form id="queryForm" name="queryForm" style="padding: 0px; margin: 0px;">
			<table id="tb_query">
				<tr>
					<th>供应商代码：</th>
					<td><input name="SUPPL_NO" id="SUPPL_NO" class="easyui-textbox" style="width: 100px"></td>

					<th>零件号：</th>
					<td><input name="PART_NO" id="PART_NO" class="easyui-textbox" style="width: 100px"></td>

					<th>配送单号：</th>
					<td><input name="MAP_SHEET_NO" id="MAP_SHEET_NO" class="easyui-textbox" style="width: 100px"></td>
					<th>开始时间：</th>
					<td><input name="begin_time" id="begin_time" data-options="formatter:myformatter,parser:myparser" class="easyui-datebox" style="width: 140px">
					</td>
					<th>结束时间：</th>
					<td><input name="end_time" id="end_time" class="easyui-datebox" data-options="formatter:myformatter,parser:myparser" style="width: 140px"></td>
...
<%=session.getAttribute("curLoginUserButton")%>
	...
	function exportData() {
		...
		Hmgx.serializeDownload("${app}/outStorage/export.action", "", params);
	}
	//查询
	function query() {
		var SUPPL_NO = $.trim($('input[name=SUPPL_NO]').val());
		...
		var end_time = $.trim($('input[name=end_time]').val());
		$("#dg").datagrid({
			url : '${app}/outStorage/list.action',
			queryParams : {
				SUPPL_NO : SUPPL_NO,
				PART_NO : PART_NO,
				MAP_SHEET_NO : MAP_SHEET_NO,
