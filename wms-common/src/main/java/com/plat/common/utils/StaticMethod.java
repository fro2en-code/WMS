...
public class StaticMethod {
    ...
    private static final AtomicLong sub = new AtomicLong(0);
    ...
    public final static String getWebappsPath() {
        String webappsPath = "";
        String classPath = StaticMethod.getClasspath();
        int pos = classPath.indexOf("webapps");
        if (pos > -1) {
            webappsPath = classPath.substring(0, pos + 8);
        }
        return webappsPath.replaceAll("%20", " ");
    }
    ...
    public static String getWebPath() throws FileNotFoundException {
        ...
        String result = StaticMethod.class.getResource("StaticMethod.class").toString();
    ...
    public static String StringReplace(String sou, String s1, String s2) {
        ...
        return StringReplace(sou.substring(0, idx) + s2 + sou.substring(idx + s1.length()), s1, s2);
    }
        ...
        for (int i = 0; i < per_page && begin <= total; i++) {
            ...
            if (current >= begin && current <= begin + per_page - 1) {
                ...
                rslt += fillValuesToString(
                        "<a href='$action" + strStart + String.valueOf(begin) + "'>[ $from - $to ]</a>&nbsp;", ht);
            }
            begin += per_page;
        }
        if (total > begin) {
    ...
    public static GregorianCalendar String2Cal(String strDate) {
        ...
        if (vecDate.size() > 0 && vecDate.size() < 3) {
            if (vecDate.size() == 1) {
                Vector vecData = StaticMethod.getVector(String.valueOf(vecDate.elementAt(0)), "-");
                if (vecData.size() == 3) {
                    int intYear = Integer.parseInt(String.valueOf(vecData.elementAt(0)));
                    int intMonth = Integer.parseInt(String.valueOf(vecData.elementAt(1)));
                    int intDay = Integer.parseInt(String.valueOf(vecData.elementAt(2)));
            ...
            if (vecDate.size() == 2) {
                Vector vecData = StaticMethod.getVector(String.valueOf(vecDate.elementAt(0)), "-");
                Vector vecTime = StaticMethod.getVector(String.valueOf(vecDate.elementAt(1)), ":");
                if (vecData.size() == 3) {
                    int intYear = Integer.parseInt(String.valueOf(vecData.elementAt(0)));
                    int intMonth = Integer.parseInt(String.valueOf(vecData.elementAt(1)));
                    int intDay = Integer.parseInt(String.valueOf(vecData.elementAt(2)));
                ...
                if (vecTime.size() == 3) {
                    int intHour = Integer.parseInt(String.valueOf(vecTime.elementAt(0)));
                    int intMinute = Integer.parseInt(String.valueOf(vecTime.elementAt(1)));
                    int intSecond = Integer.parseInt(String.valueOf(vecTime.elementAt(2)));
    ...
    public static String Cal2String(GregorianCalendar calDate) {
        ...
        strDate = strDate + "-" + String.valueOf(calDate.get(Calendar.MONTH) + 1);
        strDate = strDate + "-" + String.valueOf(calDate.get(Calendar.DATE));
        strDate = strDate + " " + String.valueOf(calDate.get(Calendar.HOUR_OF_DAY));
    ...
    public static String getTimeString(int disday, int hour) {
        ...
        ls_display = "'" + _yy + "-" + _mm + "-" + _dd + " " + _hourstr + ":00:00'";
        return ls_display;
    }
    ...
    public static String getLocalString(int disday, int hour) {
        ...
        ls_display = _yy + "-" + _mmstr + "-" + _ddstr + " " + _hourstr + ":00:00";
        return ls_display;
    }
    ...
    public static int getTimeIntervals(String historyTime, String nowTime) {
        ...
        int flag, minutes;
        GregorianCalendar localCalendar = new GregorianCalendar(year1, month1, day1, hour1, min1, sec1);
        GregorianCalendar limitCalendar = new GregorianCalendar(year2, month2, day2, hour2, min2, sec2);
     ...
     * @return
     */
    public static Object invokeStringMethod(Object obj, String attributeName, String value) throws Exception {
        try {
            String setMethod = "set" + StaticMethod.firstToUpperCase(attributeName);
            Method setterMethod = obj.getClass().getMethod(setMethod, new Class[] { String.class });
    ...
    public static String getOperDay() {
        ...
        String months = String.valueOf(currentCalendars.get(Calendar.MONTH) + 1);
        String days = String.valueOf(currentCalendars.get(Calendar.DAY_OF_MONTH));
        String hours = String.valueOf(currentCalendars.get(Calendar.HOUR_OF_DAY));
        ...
        strday = years + "-" + months + "-" + days + " " + hours + ":" + mm + ":" + ss;
    ...
    public static int getByMonthToNum(String year, String month) throws SQLException {
        int num = 0;
     ...
     * @return
     */
    public static String genRandomNum2(int length) {
        StringBuilder newRandom = new StringBuilder();
        Random rd = new Random();
        for (int i = 0; i < length; i++) {
            newRandom.append(NUM_CHAR[rd.nextInt(9)]);
        }
     ...
     * @return
     */
    static public boolean isMobile(String mobile) {
        String reg = "^(13[0-9]|15[0-9]|18[0-9]|147)\\d{8}$";
        if (mobile != null && Pattern.matches(reg, mobile)) {
            return true;
        }
        return false;
    }
     ...
     * @return
     */
    public static String genImgname(String adType, String subfix) {
        String sequence = "";
        long tmp = sub.incrementAndGet() % 1000;
        sequence = adType + date() + getMicrosecond() + tmp + subfix;
        return sequence;
    }
    ...
    private static long getMicrosecond() {
        Calendar c = Calendar.getInstance();
    ...
    public static int getRandomNum(int min, int max) {
        return random.nextInt(max) % (max - min + 1) + min;// 关键在这里
    }
